...
vec3 radiance(Ray r,vec3  sunDir, vec3 sunCol)
{
    float epsilon = 4e-4;
    ...
    for (int i = 0; i <= MAX_BOUNCES; ++i)
    {
        ...
        {
            accum += attenuation * skyColor(r.d,sunDir);
            break;
        }
    }
    return accum;
...
vec3 sunDir = normalize(vec3(.8, .55, -1.));
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        Ray r = Ray(p, d);
        color += radiance(r,sunDir,sunCol) / 4.;
    }

	COLOR = vec4(Uncharted2ToneMapping(color),1.0);
}
